@extends('backend/layouts/admin-master', ['title' => 'Answer Page'])
@section('admin')
       <!-- Start Content-->
       <div class="content">
          ...
          <div class="container-fluid">
             <div class="px-1 py-3 d-flex align-items-sm-center flex-sm-row flex-column">
                <div class="flex-grow-1">
                   <h4 class="m-0 fs-18 fw-semibold">All Answer</h4>
                </div>
                <div class="text-end">
          ...
          <div class="container-fluid">
             ...
             <div class="mb-0 border card">
                <div class="card-header d-flex justify-content-between align-items-center">
                   <h4 class="mb-0 card-title">Answer List</h4>

                   <a href="{{ route('admin.answer.create') }}" class="btn btn-success float-end">Add
                    Answer</a>
                </div><!--end card-header-->
                      ...
                      <tbody>
                         ...
                         @forelse($answers as $answer)
                            <tr>
                               <td>{{ $loop->index + 1 }}</td>
                               <td>{{ $answer->title }}</td>
                               <td>{{ Str::limit($answer->description,25) }}</td>
                               <td>
                                  <a href="{{ route('admin.answer.edit', $answer) }}" class="btn btn-success">Edit</a>
                                  <form action="{{ route('admin.answer.destroy', $answer->id) }}" method="POST"
                                     class="d-inline">
                                     @csrf
                                     @method('DELETE')
                                     <button type="submit" class="btn btn-danger btn-delete"
                                        onclick="deleteConfirm($answer->id)">Delete</button>
                                  </form>
                               </td>
                            </tr>
                         @empty
