@extends('frontend/layouts/frontend-master', ['title' => 'Single Blog'])
...
alt="Blog Image" style="width: 100%; height: 500px;">
</div>
...
<!-- comment-box -->
<div class="lonyo-blog-d-comment-box" id="commentSection">
<h4>Comments ({{ $blog->comments->count() }})</h4>
{{-- Comment List --}}
@foreach($blog->comments->whereNull('parent_id') as $comment)
<div class="my-3 lonyo-blog-d-comment-wrap d-flex justify-content-between align-items-center">
...
<!-- 3-dot button -->
<button class="border-0 btn btn-light dropdown-toggle" type="button"
...
<li>
<button onclick="showEditBox({{ $comment->id }})"
class="dropdown-item text-success btn-outline-primary">
Edit
</button>
</li>
...
class="dropdown-item text-danger" onclick="deleteConfirmation(event)">
Remove
</a> --}}
...
<!-- Reply Button -->
<button onclick="ReplyForm({{ $comment->id }})" class="reply-btn">
Reply
</button>
</div>
</div>
...
<div class="my-2 pl-101" id="reply-form-{{ $comment->id }}" class="mt-2" style="display:none;">
<input type="hidden" id="blog_id" value="{{ $blog->id }}">
<input type="hidden" id="parent_id" value="{{ $comment->id }}">

<textarea id="reply-text-{{ $comment->id }}" class="mb-2 form-control"
placeholder="Write reply..."></textarea>

{{-- <button type="button" onclick="ReplayComment()" class="reply-btn btn btn-success">
Post Reply
</button>--}}
<button type="button" onclick="ReplayComment({{ $comment->id }})" class="btn btn-primary">
Post Reply
</button>
</div>


<!-- Hidden Edit Box id="comment-{{ $comment->id }}" -->
<div class="my-2 mt-2 pl-101 edit-box d-none" id="editBox{{ $comment->id }}">
<textarea id="editContent{{ $comment->id }}" class="form-control" rows="2"></textarea>

<div class="mt-2">
<button onclick="updateComment({{ $comment->id }})" class="btn btn-sm btn-success">
Update
</button>

<button onclick="cancelEdit({{ $comment->id }})" class="btn btn-sm btn-secondary">
Cancel
</button>
</div>
</div>
...
<div class="reply-btn">
<button onclick="ReplyForm({{ $reply->id }})" class="reply-btn">
Reply
</button>
<button onclick="deleteComment({{ $reply->id }})"
class="w-full px-1 py-1 my-2 text-sm text-left text-white rounded bg-danger">
Remove
</button>
</div>
</div>
...
<!-- Admin Reply Form -->
<div class="mt-2 pl-101" id="reply-form-{{ $reply->id }}" style="display:none;">

<!-- Hidden Comment ID -->
<input type="hidden" id="comment_id{{ $reply->id }}" value="{{ $comment->id }}">

<!-- Reply Textarea -->
<textarea id="reply_text_{{ $reply->id }}" 
class="mb-2 form-control" 
placeholder="Admin reply..."></textarea>

<!-- Submit Button -->
<button type="button" 
onclick="AdminReply({{ $reply->id }})" 
...
<form action=" {{ route('comment.reply') }}"
method="POST" class="mt-2">
@csrf
<input type="hidden" name="comment_id" value="{{ $comment->id }}">
<textarea name="reply" class="mb-1 form-control"
placeholder="Admin reply..."></textarea>
...
<!-- End Admin Reply Form -->


<!-- Admin Reply Button -->
{{-- @if(auth()->user()->is_admin) --}}
{{-- <button type="button" class="mt-2 text-blue-500 hover:underline"
onclick="toggleReplyBox({{ $comment->id }})">
Reply as Admin
</button>

<!-- Admin Reply Box (Hidden by default) -->
<div class="hidden mt-2 admin-reply-box" id="reply-box-{{ $comment->id }}">
<input type="hidden" id="comment_id-{{ $comment->id }}" value="{{ $comment->id }}">

<textarea id="reply_id-{{ $comment->id }}" class="w-full p-2 mt-1 border rounded"
placeholder="Write admin reply..."></textarea>

<button type="button" class="mt-1 btn btn-success"
onclick="AdminReply({{ $comment->id }})">
Reply
</button>
</div> --}}
{{-- @endif --}}
...
<textarea class="button-text" name="comment" id="comment"
placeholder="Write a comment..."></textarea>
</div>
<button class="lonyo-default-btn extra-btn d-block" type="button"
...
const Admin_Reply_Url = "{{ route('comment.reply') }}";
